...
def generate(folder, bin_folder, bin_painting_folder):
    ...
    for i in range(len(file_list)):
        ...
        # image = inference_segmentor(model, image_file)
        # import pdb
        # pdb.set_trace()
        ...
        with torch.no_grad():
            image_feature = model.inference(data['img'][0], data['img_metas'][0], True)
        points = np.fromfile(bin_file, dtype=np.float32).reshape([-1,4])  # n*4
        seg_info = np.zeros([points.shape[0], 1], dtype = np.float32)  # n*1
        points_painting = np.column_stack((points, seg_info))  # n*5
        ...
        for j in range(pixel_xy.shape[0]):
            if pixel_xy[j][0] >= 0 and pixel_xy[j][0] < 1280 \
                ...
                # if image_feature[0][pixel_xy[j][1], pixel_xy[j][0]] == 11:
                #     points_painting[j, 4] = 1.0
                points_painting[j, 4] = image_feature[0, 11, pixel_xy[j][1], pixel_xy[j][0]]
        # import pdb
        # pdb.set_trace()
...
model = init_segmentor(config_file, checkpoint_file,
                            device='cuda:3')  # TODO edit here if you want to use different device
